...
export default function ClientHome() {
  ...
  return <>
    ...
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8"><div className="mb-6 flex flex-col gap-2 min-[400px]:flex-row min-[400px]:items-end min-[400px]:justify-between"><div><h2 className="th-display text-2xl font-medium">Estabelecimentos</h2><p className="mt-1 text-sm text-[#5C6B68]">Escolha onde você quer ser atendido.</p></div><span className="text-sm text-[#5C6B68]">{filtered.length} encontrados</span></div>
      {loading ? <div className="flex justify-center py-20"><Loader2 className="h-6 w-6 animate-spin text-[#0F5C56]" /></div> : error ? <div className="rounded-2xl border border-red-200 bg-red-50 p-5 text-sm text-red-700">Não foi possível carregar os estabelecimentos.</div> : filtered.length === 0 ? <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-[#E4E1D8] py-20"><Store className="h-6 w-6 text-[#5C6B68]" /><p className="text-sm text-[#5C6B68]">Nenhum estabelecimento encontrado.</p></div> : <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{filtered.map((item) => <Link key={item.id} to={`/estabelecimentos/${item.id}/agendar`} className="group min-w-0 overflow-hidden rounded-2xl border border-[#E4E1D8] bg-white transition hover:-translate-y-0.5 hover:shadow-lg"><div className="flex h-36 items-center justify-center bg-[#0F5C56]/8">{item.photo ? <img src={item.photo} alt="" className="h-full w-full object-cover" /> : <Store className="h-9 w-9 text-[#0F5C56]/45" />}</div><div className="p-5"><h3 className="text-lg font-semibold group-hover:text-[#0F5C56]">{item.name}</h3><p className="mt-2 flex min-w-0 items-start gap-1.5 break-words text-sm text-[#5C6B68]"><MapPin className="h-4 w-4 shrink-0" />{item.address || "Endereço não informado"}</p><div className="mt-4 flex items-center justify-between border-t border-[#E4E1D8] pt-4 text-xs text-[#5C6B68]"><span className="flex items-center gap-1.5"><Scissors className="h-3.5 w-3.5" />{item.service_count} serviços</span><span className="flex items-center gap-1.5"><Clock3 className="h-3.5 w-3.5" />{item.starting_price === null ? "Consulte" : `A partir de ${formatCurrency(item.starting_price)}`}</span></div></div></Link>)}</div>}
    </section>
  </>;
}
