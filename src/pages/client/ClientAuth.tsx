...
import { z } from "zod";
import { createClientAppointment, loginClient, registerClient } from "../../api/client";
...
export default function ClientAuth() {
  ...
  async function submit(values: FormData) {
    ...
    try {
      ...
      const pending = storage.getPendingBooking();
      if (pending) {
        try {
          await createClientAppointment({ collaborator_id: pending.collaborator_id, service_id: pending.service_id, appointment_date: pending.appointment_date, start_time: pending.start_time });
          storage.clearPendingBooking();
          navigate("/meus-agendamentos", { state: { booked: true } });
          return;
        } catch {
          storage.clearPendingBooking();
          navigate(`/estabelecimentos/${pending.establishment_id}/agendar`, { state: { bookingError: "O horário escolhido não está mais disponível. Selecione outro." } });
          return;
        }
      }
      navigate("/");
  ...
  return <div className="flex min-h-screen bg-[#F7F6F2]">
    ...
    <main className="flex flex-1 items-center justify-center px-6 py-12"><div className="w-full max-w-sm"><Link to="/" className="mb-10 flex items-center gap-2 lg:hidden"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#0F5C56] text-white"><CalendarCheck2 className="h-4 w-4" /></span><span className="th-display text-xl font-semibold">tem horário?</span></Link><h2 className="th-display text-3xl font-medium">{isRegister ? "Crie sua conta" : "Que bom ter você de volta"}</h2><p className="mt-2 text-sm text-[#5C6B68]">{isRegister ? "Cadastre-se para confirmar seus agendamentos." : "Entre para agendar e acompanhar seus horários."}</p>
      ...
      {storage.getPendingBooking() && <p className="mt-4 rounded-xl bg-[#0F5C56]/8 p-3 text-center text-xs text-[#0F5C56]">Após entrar, tentaremos confirmar automaticamente o horário escolhido.</p>}
